...
export function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'rich-text-content focus:outline-none min-h-[60px] px-2 py-1.5',
      },
    },
  });
  ...
  return (
    <div className={`rich-text-editor relative border border-border rounded-lg bg-background flex flex-col ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}>
      {!disabled && (
        <div className="flex items-center gap-1 p-1.5 border-b border-border bg-surfaceLight rounded-t-lg flex-wrap flex-shrink-0">
      ...
      <div className={`relative flex-1 min-h-0 ${hasMaxHeight ? 'overflow-y-auto scrollbar-thin' : ''}`}>
        <EditorContent editor={editor} />
        {isEmpty && !disabled && (
          <div className="absolute pointer-events-none text-textMuted text-xs px-2 py-1.5" style={{ top: 0, left: 0 }}>
            {placeholder}
          </div>
        )}
      </div>
